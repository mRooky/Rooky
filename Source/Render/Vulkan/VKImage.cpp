...
#include "../../Core/Math/MathColorRGBA.h"
...
VkClearValue Image::GetClearValue(void) const
{
	...
	{
		Math::ColorRGBA clear_color(mLayout.GetClearColor());
		std::cout << "Clear Color:";
		std::cout << clear_color.ToString();
		std::cout << std::endl;

		clear_value.color.float32[0] = clear_color.rgb.red;
		clear_value.color.float32[1] = clear_color.rgb.green;
		clear_value.color.float32[2] = clear_color.rgb.blue;
		clear_value.color.float32[3] = clear_color.alpha;
	}
	return clear_value;
}
